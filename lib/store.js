  ...
  persist(
    (set, get) => ({
      ...
      saveDayToHistory: () => {
        ...
        const completionRate = todayData.completedActivities.length / todayActivities.length;
        
        // Adicionar métricas de performance ao dia
        dayData.metrics = {
          completionRate: completionRate * 100,
          totalActivities: todayActivities.length,
          completedActivities: state.todayData.completedActivities.length,
          dayOfWeek: dayOfWeek
        };
      ...
      analyzeEvolution: () => {
        ...
        const dayBeforeYesterday = history[history.length - 2];
        
        // Análise de sono
        const avgSleepHours = last7Days.reduce((acc, day) => acc + (day.sleepHours || 0), 0) / last7Days.length;
        const sleepTrend = yesterday.sleepHours > dayBeforeYesterday.sleepHours ? 'improving' : 'declining';
        
        // Análise de energia
        const avgEnergy = last7Days.reduce((acc, day) => acc + (day.energy || 0), 0) / last7Days.length;
        const energyTrend = yesterday.energy > dayBeforeYesterday.energy ? 'improving' : 'declining';
        
        // Análise de conclusão
        const avgCompletion = last7Days.reduce((acc, day) => acc + (day.metrics?.completionRate || 0), 0) / last7Days.length;
        ...
        if (avgSleepHours < 7) {
          problems.push({
            type: 'sleep',
            severity: 'high',
            message: 'Média de sono abaixo de 7 horas',
            recommendation: 'Estabeleça horário fixo para dormir'
          });
        }
        ...
        if (avgEnergy < 60) {
          problems.push({
            type: 'energy',
            severity: 'medium',
            message: 'Energia média baixa',
            recommendation: 'Revisar nutrição e hidratação'
          });
        }
        ...
        if (avgCompletion < 70) {
          problems.push({
            type: 'completion',
            severity: 'high',
            message: 'Taxa de conclusão abaixo do ideal',
            recommendation: 'Priorizar atividades essenciais'
          });
        }
        
        // Identificar melhorias
        if (yesterday.sleepHours > 7.5 && dayBeforeYesterday.sleepHours <= 7.5) {
          improvements.push({
            type: 'sleep',
            message: 'Melhora na qualidade do sono'
          });
        }
        
        if (yesterday.energy > dayBeforeYesterday.energy + 10) {
          improvements.push({
            type: 'energy',
            message: 'Aumento significativo de energia'
          });
        }
        
        // Gerar metas adaptativas para hoje
        const adaptiveGoals = [];
        ...
        if (avgSleepHours < 7) {
          adaptiveGoals.push({
            ...
            message: 'Dormir pelo menos 8 horas'
          });
        }
        ...
        if (avgCompletion < 80) {
          adaptiveGoals.push({
            type: 'completion',
            target: Math.min(avgCompletion + 10, 100),
            current: avgCompletion,
            message: 'Aumentar taxa de conclusão em 10%'
          });
        }
        ...
        return {
          ...
          yesterday,
          comparison: {
            sleepDiff: yesterday.sleepHours - dayBeforeYesterday.sleepHours,
            energyDiff: yesterday.energy - dayBeforeYesterday.energy,
            completionDiff: (yesterday.metrics?.completionRate || 0) - (dayBeforeYesterday.metrics?.completionRate || 0)
          }
        };
      },
